...
def generate_eng_question(d, eng_idx, used):
    ...
    if qtype == "vocab":
        ...
        return {
            "type":     "grammar",
            "text":     f'Is this sentence grammatically **correct** or **incorrect**?\n\n> *"{sent}"*',
        ...
        return {
            "type":     "sentence",
            "text":     f'What type of sentence is this?\n\n> *"{sent}"*',
...
def show_game():
    ...
    # ── Question text ──
    st.markdown(f"<h3 style='margin-bottom:0'>{s.q['text']}</h3>", unsafe_allow_html=True)
    st.divider()
